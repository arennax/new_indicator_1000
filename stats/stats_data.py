...
def data_stats(path):
    for filename in os.listdir(path):
        if not filename.startswith('.'):
            repo_pool.append(os.path.join(filename))
    data_sample = pd.read_csv(path + repo_pool[0], sep=',').drop(columns=['dates'])
    ...
    for feature in range(n_column):
        ...
        for repo in repo_pool:
            df_raw = pd.read_csv(path + repo, sep=',').drop(columns=['dates'])
        ...
        with open("../result_stats/stats_data.csv", "a+") as output:

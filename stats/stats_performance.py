...
def repo_stats(filename, path):
    ...
    n_column = len(result_raw.columns)
    for i in range(n_column - 1):
        result_win.append(0)

    for learner in range (1, n_column):
    ...
    win_range = np.quantile(result_raw.iloc[:, 1:n_column].to_numpy(), 0.424)
    ...
    with open("../result_stats/stats_performance.csv", "a+") as output:
